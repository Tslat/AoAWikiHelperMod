...
public class CommandPrintCannonsOverview extends CommandBase {
	@Override
	public String getName() {
		return "printcannonsoverview";
	}

	@Override
	public String getUsage(ICommandSender sender) {
		return "/printcannonsoverview [clipboard] - Prints out all AoA cannons data to file. Optionally copy contents to clipboard.";
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
		...
		if (!world.isRemote) {
			...
			for (BaseCannon cannon : cannons) {
				...
				String name = cannon.getItemStackDisplayName(cannonStack);
				String unholsterTime = StringUtil.roundToNthDecimalPlace(1 / (((int)(400 * (1 - (-ItemUtil.getStackAttributeValue(cannonStack, SharedMonsterAttributes.ATTACK_SPEED, (EntityPlayer)sender, EntityEquipmentSlot.MAINHAND, AoAAttributes.ATTACK_SPEED_MAINHAND)) / 100f))) / 100f), 2) + "s";
				String firingSpeed = (2000 / cannon.getFiringDelay()) / (double)100 + "/sec";

				data.add("| [[File:" + name + ".png|64px|link=]] '''[[" + name + "]]''' || {{hp|" + StringUtil.roundToNthDecimalPlace((float)cannon.getDamage(), 1) + "}} || " + unholsterTime + " || " + firingSpeed + " || " + StringUtil.roundToNthDecimalPlace(cannon.getRecoil(), 1) + " || " + cannon.getMaxDamage(cannonStack) + " || ");
				data.add("|-");
			}

			data.add("|}");
			CategoryTableWriter.writeData("Cannons", data, sender, copyToClipboard);
		}
	}
}
