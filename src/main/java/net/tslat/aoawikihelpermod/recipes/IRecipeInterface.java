...
public abstract class IRecipeInterface {
    ...
    protected abstract String buildIngredientSummaryLine(ItemStack targetStack, ArrayListMultimap<String, IRecipe> matchedRecipes);
    ...
    protected String buildSummaryLine(ItemStack targetStack, ArrayListMultimap<String, IRecipe> matchedRecipes) {
        return "'''[[" + recipe.getRecipeOutput().getDisplayName().getString() + "]]''' || " + buildIngredientSummaryLine(targetStack, matchedRecipes) + " || {{" + getWikiTemplateName();
    }

    public int sortingCompare(IRecipeInterface comparingInterface) {
        return recipe.getRecipeOutput().getDisplayName().getString().compareTo(comparingInterface.recipe.getRecipeOutput().getDisplayName().getString());
    }
