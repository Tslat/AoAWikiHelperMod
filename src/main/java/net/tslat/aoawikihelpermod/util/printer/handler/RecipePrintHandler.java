...
public abstract class RecipePrintHandler {
	...
	public static class RecipeIngredientsHandler {
		...
		public void addIngredientsToWikiTemplate(WikiTemplateHelper.Template template) {
			for (int index = 0; index < slotPrefixes.length; index++) {
				if (index >= this.ingredientsIndex.size())
					return;

				int ingredientIndex = this.ingredientsIndex.get(index);

				if (ingredientIndex == -1)
					continue;

				PrintableIngredient ingredient = this.ingredients.get(ingredientIndex);

				if (ingredient == PrintableIngredient.EMPTY)
					continue;

				String slotPrefix = slotPrefixes[index];

				if (ingredient.imageName != null) {
					template.entry(slotPrefix + "image", ingredient.imageName)
							.entry(slotPrefix, ingredient.imageName.substring(0, ingredient.imageName.lastIndexOf(".")));
				}
				else {
					template.entry(slotPrefix, ingredient.formattedName);
				}
			}
		}
		...
		public String getFormattedIngredient(PrintableIngredient ing, @Nullable Item targetItem) {
			if (ing.isTag())
				return FormattingHelper.createLinkableTag(ing.formattedName, Registries.ITEM);
