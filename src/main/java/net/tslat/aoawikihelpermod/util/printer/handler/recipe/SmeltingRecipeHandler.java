...
public class SmeltingRecipeHandler extends RecipePrintHandler {
	...
	public String[] toTableEntry(@Nullable Item targetItem) {
		...
		printData[3] = input.formattedName.contains(":") ? FormattingHelper.createTagIngredientDescription(input.formattedName, Registries.ITEM) : FormattingHelper.createLinkableText(input.formattedName, false, !input.matches(targetName));
		printData[4] = WikiTemplateHelper.makeSmeltingTemplate(input, output);
